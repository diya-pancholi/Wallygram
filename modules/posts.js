class Posts
{
    ...
    getFeedComments()
    {
      const result = (this.db).query(`SELECT comments.Caption, comments.post_id, comments.username FROM comments INNER JOIN posts ON posts.Post_id=comments.post_id WHERE posts.Username IN (SELECT friends_username From friends WHERE Username = "${this.username}") OR posts.Username IN (SELECT Username From friends WHERE friends_username = "${this.username}");`);
      return result;
    }
